...
class Lside extends React.Component {
    constructor(props){
        super(props)
        this.state = {
            collapsed: false,
    ...
    render() {
        ...
        const menusLinks = this.props.menuName.map((item) => {

            const menus = Object.keys(item.menus_text).map(keys => {
                return (
                    <Menu.Item key={item.menus_text[keys].text}> <
                        Link to={item.menus_text[keys].links}>{item.menus_text[keys].text}</Link>
                    </Menu.Item>
                )
            })
            return (
                <SubMenu key={item.menu_title[0]} title={<span><Icon type={item.menu_title[1]}/>
                         <span>{item.menu_title[0]}</span>
                     </span>}>
                    {menus}
                </SubMenu>
            )
